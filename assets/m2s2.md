...
1. **Mapeo de los proveedores líderes**

   * Reconoce al menos tres grandes proveedores: Amazon Web Services (AWS), Microsoft Azure y Google Cloud Platform (GCP).
   * Incluye también opciones especializadas o emergentes: IBM Cloud, Oracle Cloud, DigitalOcean, y proveedores de nicho (p. ej. Linode, Vultr).
   * Trabaja autónomamente investigando sus webs oficiales y comparativas independientes.

2. **Comparativa de modelos de servicio (IaaS, PaaS, SaaS)**

   * Identifica qué niveles ofrece cada proveedor: infraestructura (IaaS), plataforma (PaaS) o aplicaciones (SaaS).
   * Anota ejemplos concretos: EC2 vs. Azure Virtual Machines vs. Compute Engine.
   * En equipo, discute ventajas y limitaciones de cada modelo según el caso de uso en ciberseguridad (p. ej. análisis forense, honeypots).

3. **Evaluación de precios y modelos de facturación**

   * Analiza los esquemas de pago por uso, instancias reservadas y descuentos por volumen.
   * Compara costos de almacenamiento, transferencia de datos y cómputo.
   * Autonomía: usa calculadoras de precios; Colaboración: comparte hojas de cálculo con escenarios de ejemplo.

4. **Cobertura geográfica y zonas de disponibilidad**

   * Identifica la cantidad de regiones y zonas de cada proveedor para asegurar redundancia y baja latencia.
   * Destaca la importancia en ciberseguridad de elegir ubicaciones con certificaciones de privacidad/localización de datos.
   * En grupo, elabora un mapa colaborativo (por ejemplo, en Miro o Google Slides) sobre ubicaciones clave.

5. **Características de seguridad nativas**

   * Revisa servicios como firewalls, WAF, gestión de identidades (IAM), cifrado en reposo y en tránsito.
   * Identifica diferencias/clasificaciones: AWS Shield vs. Azure DDoS Protection vs. GCP Cloud Armor.
   * Trabaja en parejas para crear mini-casos prácticos donde se active y configure cada servicio.

6. **Herramientas de gestión y orquestación**

   * Compara consolas web, CLI (AWS CLI, Azure CLI, gcloud) y APIs.
   * Explora frameworks de IaC (Terraform, AWS CloudFormation, Azure Resource Manager).
   * Autonomía: practica scripts IaC; Colaboración: aloja el código en un repositorio Git compartido.

7. **Ecosistema de servicios complementarios**

   * Analiza servicios de contenedores (EKS, AKS, GKE), serverless (Lambda, Functions) y bases de datos (RDS, Cosmos DB, Cloud SQL).
   * Evalúa la integración entre ellos y su impacto en la arquitectura segura.
   * Forma equipos para diseñar una arquitectura de referencia que incluya varios servicios de un mismo proveedor.

8. **Soporte, documentación y certificaciones**

   * Consulta la calidad y amplitud de la documentación oficial y los foros/community support.
   * Revisa planes de soporte (gratuito, Developer, Enterprise) y tiempos de respuesta.
   * Colaborativamente, organiza sesiones “lunch & learn” donde cada miembro exponga un hallazgo relevante.

9. **Conformidad, normativas y certificaciones**

   * Identifica certificaciones clave: ISO 27001, SOC 2, GDPR, HIPAA, FedRAMP.
   * Analiza qué proveedor ofrece mayor cobertura para el sector público o financiero, según necesidades del diplomado.
   * Realiza autónomamente una tabla comparativa y compártela en la plataforma de aprendizaje.

10. **Evaluación práctica y feedback continuo**

    * Diseña laboratorios prácticos donde se despliegue infraestructura mínima segura en cada proveedor.
    * Documenta en equipo los pasos, retos y buenas prácticas encontradas.
    * Implementa retrospectivas periódicas para ajustar la estrategia de identificación y análisis, fomentando así la mejora continua tanto individual como grupal.

Este decálogo te permitirá, tanto de manera **autónoma** (investigación individual, uso de herramientas y laboratorios) como **colaborativa** (discusiones en equipo, repositorios compartidos, presentaciones cruzadas), comprender y comparar a fondo los principales proveedores de infraestructura en la nube, sus características y servicios, con un enfoque claro en ciberseguridad.
